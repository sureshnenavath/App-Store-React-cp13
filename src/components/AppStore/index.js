...
class AppStore extends Component {
  state = {
    getTabId: tabsList[0].tabId,
    searchInput: '',
  }

  onHandleTabClick = id => {
    const {getTabId} = this.state
    this.setState({getTabId: id})
  }
  ...
  filterData = () => {
    const {getTabId, searchInput} = this.state
    const searchResults = appsList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchResults.filter(eachItem => eachItem.category === getTabId)
  }

  render() {
    const {searchInput, getTabId} = this.state
    console.log(searchInput)
    const filteredAppList = this.filterData()
    return (
        ...
        <div className="container">
          ...
          <ul className="tab-item-ul">
            {tabsList.map(eachTabList => (
              <TabItem
                ...
                isActive={eachTabList.tabId === getTabId}
              />
            ))}
          </ul>
          ...
          <ul className="app-item-ul">
            {filteredAppList.map(eachAppList => (
              <AppItem eachAppList={eachAppList} key={eachAppList.appId} />
            ))}
          </ul>
        </div>
      </div>
